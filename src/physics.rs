...
///
/// cor2 = sağ alt köşe
#[pyclass(get_all, set_all, module = "termrender")]
...
impl AABB {
    ...
    pub fn new(start: Vec2, end: Vec2) -> Self {
        let start: Vec2 = {
            let x: i32 = { if start.x > end.x { end.x } else { start.x } };
            let y: i32 = { if start.y > end.y { end.y } else { end.y } };
            Vec2 { x, y }
        };
        let end: Vec2 = {
            let x: i32 = { if start.x < end.x { end.x } else { start.x } };
    ...
    #[pyo3(name = "is_colliding")]
    pub fn py_is_colliding(&self, other: &AABB) -> bool {
        <AABB as CollisionCheck>::is_colliding(self, other)
    }
...
impl From<((i32, i32), (i32, i32))> for AABB {
    fn from(value: ((i32, i32), (i32, i32))) -> Self {
        AABB {
            cor1: Vec2::from(value.0),
            cor2: Vec2::from(value.1),
        }
    }
}
