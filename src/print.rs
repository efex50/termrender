...
use crate::{
    Ret, RetType,
    game::screen::{CursorMoveTo, Screen},
    math::Vec2,
    physics::AABB,
};
...
#[pyclass(name = "GColor", module = "termrender")]
#[derive(Clone, Copy)]
pub struct PyGColor {
    pub inner: GColor,
}
...
impl From<GColor> for RGB {
    fn from(value: GColor) -> Self {
        match value {
            ...
            GColor::RGB(r, g, b) => Self {
                r,
                g,
                b,
                res: false,
            },
            GColor::ResetColor => Self {
                r: 0,
                g: 0,
                b: 0,
                res: true,
...
pub fn ansi_to_gcolor(code: u8) -> GColor {
    match code {
        ...
        16..=231 => {
            ...
            let g = (n % 36) / 6;
            let b = n % 6;
            GColor::RGB(r * 51, g * 51, b * 51)
        }

        // --- Grayscale ramp (232..=255) ---
        232..=255 => {
            let level = (code - 232) * 10 + 8;
            GColor::RGB(level, level, level)
        }
    }
}
...
#[pyclass(name = "GameTexture", module = "termrender")]
#[derive(Clone, Default)]
pub struct PyGameTexture {
    pub inner: GameTexture,
}

#[pymethods]
impl PyGameTexture {
    #[new]
    pub fn new(textures: Vec<PyPrintType>) -> Self {
        let inner_textures: Vec<PrintTypes> = textures.into_iter().map(|p| p.inner).collect();
        PyGameTexture {
            inner: GameTexture::new(inner_textures),
        }
    }
}
...
#[pyclass(name = "PrintType", module = "termrender")]
#[derive(Clone)]
pub struct PyPrintType {
    pub inner: PrintTypes,
}

#[pymethods]
impl PyPrintType {
    #[staticmethod]
    pub fn new_string(x: i32, y: i32, char: PyTermPrint) -> Self {
        PyPrintType {
            inner: PrintTypes::String {
                pos: Vec2::new(x, y),
                char: char.inner,
            },
        }
    ...
    #[staticmethod]
    pub fn new_line(start_x: i32, start_y: i32, end_x: i32, end_y: i32, char: PyTermPrint) -> Self {
        PyPrintType {
            inner: PrintTypes::Line {
    ...
    #[staticmethod]
    // AABB is bound as an object, here we construct it from points
    pub fn new_square(
        start_x: i32,
        start_y: i32,
        end_x: i32,
        ...
        PyPrintType {
            inner: PrintTypes::Square {
                pos: AABB::new(Vec2::new(start_x, start_y), Vec2::new(end_x, end_y)),
                char: char.inner,
            },
        }
    }
...
impl PrintTypes {
    pub fn print(&self, out: &mut Screen, _pos: Vec2) -> Ret {
        match self {
            PrintTypes::Line { start, end, char } => {
                Self::line_print(out, *start + _pos, *end + _pos, char)
            }
    ...
    pub fn clearself(&self, out: &mut Screen, _pos: Vec2, old: Vec2) -> RetType<bool> {
        match self {
            PrintTypes::Line { start, end, char } => {
                Self::line_clearself(out, old, *start + _pos, *end + _pos, char)
            }
    ...
    fn line_print(out: &mut Screen, start: Vec2, end: Vec2, char: &TermPrint) -> Ret {
        ...
        // horizontal
            ...
            let _start = { if start.x < end.x { start.x } else { end.x } };
    ...
    fn line_clearself(
        ...
        // vertical
            ...
            let _start = { if old.x < end_old.x { old.x } else { end_old.x } };
    ...
    fn square_print(out: &mut Screen, pos: AABB, char: &TermPrint) -> Ret {
        ...
        for r in 0..lines + 1 {
            out.queue(new.clone())?;
            out.cursor_move(CursorMoveTo::Pos((
                corners.0.x as u16,
                (corners.0.y + r) as u16,
            )))?;
            out.cursor_move(CursorMoveTo::Down(1))?;
        }
        Ok(())
...
#[pyclass(name = "TermPrint", module = "termrender")]
#[derive(Clone)]
pub struct PyTermPrint {
    pub inner: TermPrint,
}
...
impl PyTermPrint {
    #[new]
    #[pyo3(signature = (text, fore=None, back=None))]
    pub fn new(text: String, fore: Option<PyGColor>, back: Option<PyGColor>) -> Self {
        PyTermPrint {
            inner: TermPrint {
                text,
                fore: fore.map_or(GColor::Default, |c| c.inner),
                background: back.map_or(GColor::Default, |c| c.inner),
            },
        }
    }
...
impl From<(&str, RGB)> for TermPrint {
    fn from(value: (&str, RGB)) -> Self {
        Self {
            background: GColor::from(value.1),
            fore: GColor::Default,
